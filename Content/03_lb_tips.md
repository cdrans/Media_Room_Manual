...
The lightboard station offers something the greenscreen station cannot: the ability to write, draw, and annotate in real time while staying face-to-face with your audience. This changes the dynamic of recording considerably. Where the greenscreen rewards tight scripting and polished slide design, the lightboard rewards preparation of a different kind — knowing where you are going to draw, how to pace your writing, and how to use the glass as an expressive tool rather than just a surface to fill. This chapter covers the habits and decisions that help you make the most of it.

```{figure} ./figures/lightboard_example.png
:width: 80%
:alt: Image of a teacher solving an engineering mechanics problem on the lightboard to give an example of the look and feel of a Lightboard video.
:align: center

Example of the look and feel of a Lightboard Video.
```
...
:::{tip}
A useful rule of thumb: if your board plan requires erasing and restarting a section, the video is probably too long. Each recording should ideally use one continuous board layout, built up progressively, without needing to clear and begin again.
:::
...
```{figure} ./figures/spatial_planning.png
:width: 80%
:alt: Illustration showing how lightboard use shuld be planned to keep free board space for you to face the camera and talk to the audience
:align: center

Plan your board space so you have a place where the audience can see your face
```

### Divide the Board into Zones
...
Writing on the lightboard is not quite the same as writing on a whiteboard or chalkboard. The glass surface, the fluorescent markers, and the camera all introduce considerations that are worth knowing about before your first recording.

### Choosing and Using Markers

Fluorescent neon markers produce the glowing effect that makes the lightboard distinctive. A few practical points:

- **Use fresh markers.** A drying marker produces faint, uneven lines that may not be fully visible on camera, particularly for fine detail.
- **Write with a light touch.** Pressing hard against glass causes the marker tip to squeak audibly. A lighter touch produces cleaner lines and quieter recording conditions. With practice, you can write almost silently.
- **Use colour deliberately.** Different marker colours read differently on camera. Bright colours like yellow, green, and white typically read most clearly. Use colour variation to distinguish between elements — for instance, one colour for a given variable throughout a derivation — but keep the palette simple. More than three or four colours in a single video tends to look cluttered rather than organised.
- **Do not hold the marker while gesturing or pointing.** Put it down. A marker waving in the frame while you talk is visually distracting and gives the impression that you are about to write something when you are not.
...
## Clothing and Appearance for Lightboard Recording
...
- **Dark, solid colours.** Deep navy, black, charcoal, and dark greens work best. The glowing marker text needs to remain readable when it overlaps with your clothing, and it does this most clearly against a dark background. A bright green marker line all but disappears against a white shirt; against a dark navy top, it glows clearly.
- **No text, logos, or patterns.** Any print on your clothing will be visible in the recording, but will show up in reverse due to the mirroring process of the lightboard. As with the greenscreen video, narrow stripes, small checks, herringbone, and similar fine-weave patterns can cause a visual flickering effect known as moiré on camera. Plain, matte fabrics keep the viewer's attention on the content.
- **Comfortable, well-fitted clothing.** As with the greenscreen station, you will likely be recording for longer than you expect. Clothing you are at ease in allows you to focus on delivery.

:::{figure} ./figures/lb_clothing_dos.png
:name: fig-lbclothingdos
:alt: Two panels each marked with a green tick showing recommended lightboard clothing: a solid dark-tone blue collared shirt that blends in well with the backdrop; and a plain warm beige jumper that renders evenly without interference.
:width: 70%
:::

### What to Avoid

- **White or very pale colours.** These reduce the contrast between the glowing annotations and your clothing, making the text harder to read wherever it overlaps your body.
- **Highly reflective fabrics.** Shiny materials can catch the lightboard's edge lighting and create distracting bright spots on camera.
- **Heavy jewellery or accessories.** These can catch and reflect the light in unpredictable ways, and fine chains or earrings may create visual noise near your face.

:::{figure} ./figures/lb_clothing_nos.png
:name: fig-lbclothingnos
:alt: Three panels each marked with a red cross illustrating clothing to avoid on a lightboard video: a white t-shirt that does not blend into the black backdrop; a striped shirt showing moiré wave distortion and a brand name reversed by the lightboard video recording process; and a shiny metallic jacket with glaring light reflections.
:width: 100%
:::
